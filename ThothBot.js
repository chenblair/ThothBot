...
function MessageHandler(context, event) {
  ...
  if (event.messageobj.refmsgid == "authorizebutton" || event.message ==
  ...
  if (event.message.toLowerCase() == "kill") {
...
function EventHandler(context, event) {
  if (event.messageobj.refmsgid == "authorizebutton") {
    context.sendResponse("followthrough");
  }
}
...
function DbGetHandler(context, event) {
  ...
  var header = {
    "apikey": "3da26936384d4476c40c6eac52573b96",
